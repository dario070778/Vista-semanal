...
// Los eventos planificados no se borran automáticamente al finalizar.
// Si ya han pasado, dejan de mostrarse; los datos permanecen guardados.
...
function eventMatches(e,date,h){if(!e||e.disabled)return false;let spec=e;if(e.repeat&&e.repeat!=="none"){
 ...
 if(e.repeat==="daily"){} else if(e.repeat==="weekdays"&&dow>4)return false; else if(e.repeat==="weekly"&&!(Array.isArray(e.days)&&e.days.length?e.days.includes(dow):dow===startDow))return false; else if(e.repeat==="custom"&&!e.days?.includes(dow))return false;
...
DAYS.forEach((d,i)=>{const b=document.createElement("button");b.type="button";b.className="daytoggle";b.textContent=d;b.dataset.day=i;b.onclick=()=>{b.classList.toggle("on");if(eventEdit.mode==="edit"){const ev=state.events.find(x=>x.id===eventEdit.eventId);if(ev?.repeat&&ev.repeat!=="none")setEventScope("series")}};weekdayPicker.appendChild(b)});
...
function setEventScope(scope){eventEdit.scope=scope;scopeRow.querySelectorAll(".scopeBtn").forEach(x=>x.classList.toggle("on",x.dataset.scope===scope))}
function updateRepeatUI(resetDays=false){const r=eventRepeat.value;repeatBox.style.display=r==="none"?"none":"block";if(!resetDays)return;if(r==="weekdays")setWeekdayPicker([0,1,2,3,4]);else if(r==="weekly"){const current=getWeekdayPicker();if(!current.length)setWeekdayPicker([dayIndexFromISO(eventDate.value||isoDate(new Date()))])}else if(r==="daily")setWeekdayPicker([0,1,2,3,4,5,6]);else if(r==="custom")setWeekdayPicker([])}
eventRepeat.onchange=()=>{updateRepeatUI(true);if(eventEdit.mode==="edit")setEventScope("series")};eventDate.onchange=()=>{};
scopeRow.querySelectorAll(".scopeBtn").forEach(b=>b.onclick=()=>setEventScope(b.dataset.scope));
